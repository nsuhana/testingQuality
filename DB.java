...
public class DB {
	...
	DB() {
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://localhost/appartment", "root", "root");
			// Connection
			// con=DriverManager.getConnection("jdbc:mysql://localhost:3306/akshay","aki","papu");
	...
	ResultSet selectq(String q) {
		try {
			...
			// System.out.println("Name: "+o1.getString("name")+"<br/>"+"Flat
			// No.: "+o1.getString("flatno")+"<br/>"+"Contact No:
			// "+o1.getNString("phone"));
...

	{
		try {
			...
			Statement statement = con.createStatement();
			// resultSet gets the result of the SQL query
			// if(statement.executeUpdate("INSERT INTO info (flatno, name,
			// phone) VALUES ('4B','Om Makhija',9991112293)")>0)
...

	{
		try {
			...
			// if(statement.executeUpdate("INSERT INTO info (flatno, name,
			// phone) VALUES ('4B','Om Makhija',9991112293)")>0)
			// if(statement.executeUpdate("UPDATE servcost SET price=550 WHERE
			// serv='S'")>0)
			if (statement.executeUpdate(q) > 0) {
	...
	String calcBill(String flat) {
		...
		try {
			...
			while (o1.next()) {
				count++;
				text += "Name: " + o1.getString("name") + "<br/>" + "Flat No.: " + o1.getString("flatno") + "<br/>"
						+ "Contact No: " + o1.getString("phone");
			}
			if (count == 0) {
				text = "Enter flat no";
				JOptionPane.showMessageDialog(null, "Flat is either vacant or not registed for maintanace!",
						"Flat Not Found :(", JOptionPane.PLAIN_MESSAGE);
			} else {

				// ="Name: "+o1.getString("name")+"<br/>"+"Flat No.:
				// "+o1.getString("flatno")+"<br/>"+"Contact No:
				// "+o1.getNString("phone");
				o1 = selectq(
						"SELECT services.serv,price FROM info,services,servcost WHERE info.flatno=services.flatno AND services.serv=servcost.serv AND info.flatno='"
								+ flat + "'");
				text += "<br/> <br/> Bill:- <hr/>";
				double cost = 0;
				while (o1.next()) {
		...
		try {
			...
			while (o1.next()) {
				...
				try {
					...
					Statement statement = con.createStatement();
					// resultSet gets the result of the SQL query
					String q = "SELECT services.serv,price FROM info,services,servcost WHERE info.flatno=services.flatno AND services.serv=servcost.serv AND info.flatno='"
							+ flatno + "'";

					ResultSet o2 = statement.executeQuery(q);
					// System.out.println("Name:
					// "+o1.getString("name")+"<br/>"+"Flat No.:
					// "+o1.getString("flatno")+"<br/>"+"Contact No:
					// "+o1.getNString("phone"));

					while (o2.next()) {
						// text+=fullservConv(o1.getString("serv"))+"
						// Rs:"+o1.getString("price")+"/-<br/>" ;
	...
	String stats() {
		...
		try {
			...
			while (o1.next()) {
				text += "<tr><td>" + fullservConv(o1.getString("serv")) + "</td> <td>" + o1.getString("co")
						+ "</td></tr>";
			}
			text += "</table></html>";
		} catch (Exception e) {
	...
	void updAdd(String[] nfp, boolean[] choice) {
		...
		// String q="INSERT INTO `info`(`flatno`, `name`, `phone`) VALUES
		// ("+nfp[0]+","+nfp[1]+","+Integer.parseInt(nfp[2])+");";
		// String q=String.format("INSERT INTO info(flatno,name,phone)
		// VALUES('%s','%s',%)","12B","Karan",91234912);
		int f = 1;
		// String q=String.format"INSERT INTO info(flatno,name,phone)
		// VALUES('%s','%s',%s)", "Aks","22b","9999999");
		while (f == 1) {
			if (!insertq(
					"INSERT INTO info(name,flatno,phone) VALUES('" + nfp[0] + "','" + nfp[1] + "'," + nfp[2] + ")")) {
				JOptionPane.showMessageDialog(null, "Some problem occured while updating, retry again !",
						"Error Occured :(", JOptionPane.PLAIN_MESSAGE);
				f = 0;
				break;
			}
			for (int i = 0; i < lc; i++) {
				if (choice[i]) {
					if (!updateq("INSERT INTO services(flatno, serv) VALUES ('" + nfp[1] + "','" + serv[i].charAt(0)
						...
						JOptionPane.showMessageDialog(null,
								"Problem occured while updating services, Edit again with same flatno !",
								"Error Occured :(", JOptionPane.PLAIN_MESSAGE);
						f = 0;
						break;
					}
			...
			if (f == 1) {
				JOptionPane.showMessageDialog(null, "Member Succesfull Added !", "Success :)",
	...
	void updEdit(String string, boolean[] choice) {
		...
		while (f == 1) {
			...
			if (f == 1) {
				JOptionPane.showMessageDialog(null, "Member Succesfull Edited !", "Success :)",
						JOptionPane.PLAIN_MESSAGE);
			}
			f = 0;
		}
